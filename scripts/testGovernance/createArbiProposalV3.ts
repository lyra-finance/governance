...
const createArbitrumBridgeTest = async (
  ...
  const encodedData = ethers.utils.defaultAbiCoder.encode(["address", "uint256"], [toAddress, toBN("1000")]);
  proposalActions.targets.push(targetContract.address);
  proposalActions.values.push(BigNumber.from(0));
  proposalActions.signatures.push("transfer(address, uint256)");
  ...
  const retryableTicket = {
    ...
    arbTxCallValue: 0,
    maxSubmissionCost: 0,
    submissionRefundAddress: toAddress,
    valueRefundAddress: toAddress,
...
async function main(): Promise<void> {
  ...
  const encodedRootCalldata = await createArbitrumBridgeTest(arbiBridgeExecutor, lyraToken, testAdddress);
