...
// Half hard coded test for token transfers
const createOptimismBridgeCallData = async (
  ...
  const transferTx = await targetContract.populateTransaction.transfer(toAddress, toBN("1000"));
  const targets: string[] = [targetContract.address];
  const values: BigNumber[] = [BigNumber.from(0)];
  const signatures: string[] = [""];
  const calldatas: string[] = [transferTx.data as string];
...
async function main(): Promise<void> {
  ...
  const encodedRootCalldata = await createOptimismBridgeCallData(optimismBridgeExecutor, lyraToken, testAdddress);
